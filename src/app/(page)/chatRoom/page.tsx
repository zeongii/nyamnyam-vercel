...
export default function Home1() {
    ...
    const messageEndRef = useRef(null); // 메시지 컨테이너의 끝을 참조할 ref 생성
    ...
    useEffect(() => {
        ...
        const fetchChatRoomDetails = async () => {
            try {
                const data = await getChatRoomDetails(selectedChatRoomId);
                setSelectedChatRoom(data);
                setMessages(data.messages || []);
                setUnreadCount(0);

                // 읽지 않은 메시지들을 읽음 상태로 업데이트
                if (data.messages && Array.isArray(data.messages)) {
                    const unreadMessages = data.messages.filter(
                        (message) => !message.readBy?.[sender]
                    );

                    // 읽음 처리 및 상태 업데이트
                    for (const message of unreadMessages) {
                        await markMessageAsRead(message.id, sender);
                    }

                    // 읽음 상태로 업데이트된 메시지 목록 설정
                    setMessages((prev) =>
                        prev.map((msg) =>
        ...
        // 메시지 스트리밍 구독
        const unsubscribe = subscribeToChats(selectedChatRoomId, async (newMessage) => {
                ...
                if (!prevMessages.some((msg) => msg.id === newMessage.id)) {
                    ...
                    if (!newMessage.readBy?.[sender]) {
                        markMessageAsRead(newMessage.id, sender)
                            ...
                            .catch((error) => console.error('Failed to mark new message as read:', error));
                    }
                    scrollToBottom(); // 새 메시지가 추가될 때마다 아래로 스크롤
                ...
                <div className="uk-grid uk-grid-small" data-uk-grid>
                    ...
                    <div className="uk-width-2-3@l">
                        <div className="chat-box" style={{ display: 'flex', flexDirection: 'column', height: '100%', padding: 1, margin: 1 }}>
                            <div className="chat-messages-head" style={{ border: '1px solid #E0E0E0', borderRadius: '8px' }}>
                                {selectedChatRoomId ? (
                                    <div className="user-item">
                                        <div className="user-item__desc" style={{ width: '100%' }}>
                                            ...
                                            >
                                                {`${filteredChatRooms.find((room) => room.id === selectedChatRoomId)?.participants
                                                    .filter((participant) => participant !== localStorage.getItem('nickname'))
                            ...
                            {selectedChatRoomId ? (
                                <>
                                    <div className="chat-messages-body flex-1 overflow-y-auto bg-white shadow-md rounded-lg space-y-4" style={{ flexGrow: 1, padding: 3, margin: 3, backgroundColor: '#F5F5F5' }}>
                                        {messages.map((msg, index) => (
                                            ...
                                            >
                                                {msg.sender === sender ? (
                                                    <>
                                                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', paddingLeft: '8px', paddingTop: '4px', color: '#9E9E9E', justifyContent: 'space-between', height: '40px' }}>
                                                            <span style={{ visibility: countNotReadParticipants(msg) > 0 ? 'visible' : 'hidden', color: '#D18F36', fontSize: '0.8em', textAlign: 'left' }}>
                                                                {countNotReadParticipants(msg)}
                                                            </span>
                                                            <span style={{ color: '#B0B0B0', fontSize: '0.8em' }}>{formatTime(new Date(msg.createdAt))}</span>
                                                        </div>

                                                        <div className="message-box" style={{ maxWidth: '70%', padding: '8px 12px', borderRadius: '10px', backgroundColor: '#d1e7ff', textAlign: 'right' }}>
                                                    ...
                                                    <>
                                                        ...
                                                        <div className="message-box" style={{ maxWidth: '70%', padding: '8px 12px', borderRadius: '10px', backgroundColor: '#FFECB3', textAlign: 'left' }}>
                                                            <div style={{ fontSize: '0.9rem' }}>{msg.message}</div>
                                                        </div>

                                                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', paddingLeft: '8px', paddingTop: '4px', color: '#9E9E9E', justifyContent: 'space-between', height: '40px' }}>
                                                            <span style={{ visibility: countNotReadParticipants(msg) > 0 ? 'visible' : 'hidden', color: '#FFD700', fontSize: '0.8em', textAlign: 'left' }}>
                                                                {countNotReadParticipants(msg)}
                                                            </span>
                                                            <span style={{ color: '#B0B0B0', fontSize: '0.8em' }}>{formatTime(new Date(msg.createdAt))}</span>
                                                        </div>
                                        ...
                                        <div ref={messageEndRef} /> {/* 메시지 목록 끝에 ref 추가 */}
                                    </div>
                                    ...
                                    <div className="chat-messages-footer bg-gray-100 p-4 rounded-b-lg">
                                        <form onSubmit={sendMessage} className="chat-messages-form flex">
                                            <button type="button" onClick={toggleEmojiPicker} className="emoji-picker-button p-2 mr-2 border border-gray-300 rounded">
                                                😊
                                            </button>
                                            ...
                                            <button type="submit" className="bg-blue-500 hover:bg-blue-600 text-white font-semibold px-4 py-2 ml-2 rounded-lg">
                                                Send
                                            </button>
                                        </form>
                                    </div>
